...
class AbstractProduct(AbstractDescription, ABC):
    """Абстрактный класс для всех классов продуктов"""
    price: float
    count: int
    color: str
    _instances = []
    ...
    def __init__(self, name: str, desc: str, price: float, count: int, color: str = None):
        ...
        self._count = count
        self._color = color
        AbstractProduct._instances.append(self)
    ...
    def make_product(cls, name: str, desc: str, price: float, count: int):
        """Утилита создающая новый объект класса. Если созданный объект уже существует,
